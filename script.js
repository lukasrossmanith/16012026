const screens = { login: document.querySelector('#login-screen'), game: document.querySelector('#game-screen'), gallery: document.querySelector('#gallery-screen') };
...
function winGame(winningHearts) { gameFinished = true; winningHearts.forEach((index) => cells[index].classList.add('winning')); cells.forEach((cell) => { cell.disabled = true; }); setTimeout(startGallery, 720); }
...
function startGallery() { showScreen('gallery'); galleryTitle.hidden = false; carousel.innerHTML = memories.map((source, index) => `<figure class="carousel-card" style="--index: ${index}"><img src="${source}" alt="Memory ${index + 1}" /></figure>`).join(''); pagination.innerHTML = memories.map((_, index) => `<button type="button" aria-label="Show photo ${index + 1}" data-photo="${index}" disabled></button>`).join(''); carousel.classList.add('spinning'); carousel.addEventListener('animationend', finishSpin, { once: true }); if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) finishSpin(); }
