...
function winGame() { gameFinished = true; [0, 4, 8].forEach((index) => { markHeart(cells[index]); cells[index].classList.add('winning'); }); cells.forEach((cell) => { cell.disabled = true; }); setTimeout(startReveal, 720); }
board.addEventListener('click', (event) => { const cell = event.target.closest('.cell'); if (!cell || cell.disabled || gameFinished) return; markHeart(cell); moves += 1; if (moves === 3) winGame(); else computerMove(); });
function startReveal() { showScreen('reveal'); setTimeout(() => screens.reveal.classList.add('zooming'), 1000); setTimeout(startGallery, 2900); }
function startGallery() { screens.reveal.classList.remove('zooming'); showScreen('gallery'); galleryTitle.hidden = false; carousel.innerHTML = memories.map((source, index) => `<figure class="carousel-card" style="--index: ${index}"><img src="${source}" alt="Memory ${index + 1}" ${index < 3 ? '' : 'loading="lazy"'} /></figure>`).join(''); pagination.innerHTML = memories.map((_, index) => `<button type="button" aria-label="Show photo ${index + 1}" data-photo="${index}" disabled></button>`).join(''); carousel.classList.add('spinning'); carousel.addEventListener('animationend', finishSpin, { once: true }); if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) finishSpin(); }
